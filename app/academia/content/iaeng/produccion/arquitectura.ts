...
Haz ambos cálculos con tu volumen real, no con el que esperas tener en un año. La decisión de infraestructura se toma con el sistema que tienes, y se revisa cuando el volumen cambie de orden de magnitud — no antes.`,
    ...
    {
      id: 'arq-l3b',
      title: 'Prompt caching y tiering de modelos: bajar el coste sin bajar la calidad',
      type: 'reading',
      difficulty: 'profesional',
      content: `## El coste que se acumula sin que nadie lo note\n\nUn sistema con modelos que funciona bien en desarrollo (unas pocas llamadas, para probar) puede sorprender con una factura mucho más alta de lo esperado en producción — no porque algo esté roto, sino porque nadie diseñó el sistema pensando en coste desde el principio. Dos técnicas resuelven la mayor parte de ese problema sin tocar la calidad de las respuestas.\n\n## Prompt caching: no pagar dos veces por el mismo contexto\n\nCuando varias llamadas comparten un bloque largo de contexto —instrucciones del sistema, documentos de referencia, ejemplos few-shot— ese bloque se puede marcar para que el proveedor lo cachee del lado del servidor. Las siguientes llamadas que reutilicen ese mismo bloque pagan una fracción del coste normal por esos tokens, en vez del precio completo cada vez.\n\n\`\`\`python\nmensaje = client.messages.create(\n    model="claude-sonnet-4-5",\n    max_tokens=500,\n    system=[{\n        "type": "text",\n        "text": instrucciones_largas_del_sistema,  # varios miles de tokens, iguales en cada llamada\n        "cache_control": {"type": "ephemeral"},\n    }],\n    messages=[{"role": "user", "content": pregunta_del_usuario}]\n)\n\`\`\`\n\nEl cache tiene un tiempo de vida corto (minutos, no horas), así que el ahorro real depende de cuántas llamadas caen dentro de esa ventana con el mismo prefijo — un sistema con tráfico bajo y esporádico no se beneficia igual que uno con llamadas frecuentes sobre el mismo contexto base.\n\n## Tiering de modelos: no todo necesita el modelo más caro\n\nNo toda consulta requiere el modelo de mayor capacidad. Un router liviano —a veces tan simple como una clasificación por palabras clave, a veces un modelo pequeño y barato dedicado solo a clasificar— decide qué modelo atiende cada caso:\n\n\`\`\`python\ndef elegir_modelo(consulta: str) -> str:\n    complejidad = clasificar_complejidad(consulta)  # heurística barata o modelo pequeño\n    if complejidad == "simple":\n        return "claude-haiku-4-5"      # rapido y barato: FAQ, clasificacion, extraccion simple\n    return "claude-sonnet-4-5"          # razonamiento complejo, tareas de varios pasos\n\`\`\`\n\nEl ahorro de enrutar el 70% del tráfico —típicamente el más simple y repetitivo— a un modelo más barato suele superar ampliamente el coste de mantener la lógica de clasificación adicional.\n\n## La regla antes de optimizar coste\n\nIgual que con performance: mide antes de optimizar. Sin datos de qué consultas cuestan más y con qué frecuencia ocurren, tiering y caching se implementan a ciegas, sobre suposiciones que pueden estar equivocadas. El módulo de Evaluación y observabilidad de esta misma área ya te da la dimensión de coste por llamada — es el dato de entrada de esta lección, no algo que se estima de memoria.`,
      tasks: [
        'Identifica en tu sistema un bloque de contexto que se repite en la mayoría de las llamadas — es candidato a prompt caching',
        'Diseña el router de tiering: qué hace que una consulta sea "simple" en tu caso concreto',
        'Calcula el ahorro estimado si el 50% de tu tráfico bajara al modelo más barato',
      ],
      tip: 'El tiering mal calibrado es peor que no tener tiering: un router que manda consultas complejas al modelo barato produce respuestas peores sin que se note en las métricas de coste — solo en la calidad, que es más difícil de medir automáticamente.',
      completed: false,
    },
    {
      id: 'arq-l3c',
      title: 'Rate limiting y backpressure: protegerte de tu propio tráfico',
      type: 'reading',
      difficulty: 'profesional',
      content: `## Protegerte de tu propio tráfico\n\nLas lecciones anteriores de esta área cubrieron qué hacer cuando UN PROVEEDOR te limita a ti (backoff con jitter ante un 429). Esta es la dirección inversa: proteger TU sistema y sus dependencias downstream de una carga que tu propio tráfico genera —un pico de usuarios, un bug en un cliente que reintenta sin control, un scraper.\n\n## Rate limiting en tu propia API\n\nSin un límite explícito, un solo cliente mal comportado (o malicioso) puede consumir el presupuesto completo de llamadas al modelo que compartís entre todos los usuarios. Un limitador por identidad de usuario —no solo por IP, que un usuario legítimo puede compartir con otros en una red corporativa— es la primera línea de defensa:\n\n\`\`\`python\nfrom collections import defaultdict\nimport time\n\n# Ejemplo minimo: token bucket por usuario, en memoria\n# (en produccion real, esto vive en Redis, no en un dict local)\nbuckets: dict[str, list[float]] = defaultdict(list)\n\ndef permitir(usuario_id: str, limite: int = 10, ventana_seg: int = 60) -> bool:\n    ahora = time.time()\n    peticiones = buckets[usuario_id]\n    peticiones[:] = [t for t in peticiones if ahora - t < ventana_seg]\n    if len(peticiones) >= limite:\n        return False  # limite alcanzado, rechazar o encolar\n    peticiones.append(ahora)\n    return True\n\`\`\`\n\n## Backpressure: qué hacer cuando llega más de lo que podés procesar\n\nRate limiting rechaza directamente. **Backpressure** es la alternativa cuando rechazar de plano no es aceptable —el sistema le dice a quien pide que espere, en vez de fallar. Una cola con un tamaño máximo cumple ese rol: mientras hay espacio, encola y procesa en orden; cuando se llena, recién ahí empieza a rechazar o a devolver una señal explícita de "reintenta más tarde", en vez de aceptar trabajo que nunca va a poder atender a tiempo.\n\n## Circuit breaker: dejar de intentar cuando algo ya sabés que está roto\n\nSi una dependencia downstream (el proveedor del modelo, una base de datos, otro servicio) empieza a fallar de forma sostenida, seguir intentando cada petición individual —incluso con backoff— retrasa cada respuesta y desperdicia recursos en llamadas que van a fallar de todos modos. Un circuit breaker cuenta fallos recientes y, al superar un umbral, **deja de intentar** durante una ventana de tiempo, devolviendo un error inmediato en vez de esperar un timeout en cada petición. Pasado ese tiempo, deja pasar una petición de prueba para ver si la dependencia se recuperó antes de reabrir el flujo completo.\n\n## Cuál de los tres necesitás\n\nRate limiting protege tu sistema de EXCESO de tráfico legítimo o no. Backpressure evita que aceptes más trabajo del que podés procesar sin fallar en silencio. Circuit breaker evita insistir contra una dependencia que ya sabés que está caída. Los tres son complementarios, no alternativos — un sistema en producción real casi siempre necesita los tres, aplicados en capas distintas.`,
      tasks: [
        'Implementa un rate limit por usuario (no por IP) en un endpoint tuyo que llama a un modelo',
        'Diseña qué pasa en tu sistema cuando la cola de trabajo se llena: ¿rechaza, encola, o falla en silencio hoy?',
        'Explica con un ejemplo propio cuándo un circuit breaker evitaría un problema que backoff con reintentos no evita',
      ],
      tip: 'Un rate limit sin mensaje de error claro para el usuario se percibe como que el producto "no funciona", no como una protección razonable. Siempre que limites, devolvé una respuesta explícita — código de estado 429 y un mensaje que diga cuándo puede reintentar.',
      completed: false,
    },
    {
      id: 'arq-l4',

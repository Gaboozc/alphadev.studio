...
Esta comprobación es barata y cubre el caso límite donde, por la naturaleza de "al menos una entrega" de cualquier sistema distribuido, un mismo evento llega dos veces pese a la estrategia de reconexión. Es la misma idempotencia del módulo de procesamiento asíncrono, aplicada del lado del cliente.`,
    ...
    {
      id: 'realtime-l3b',
      title: 'Renderizar el stream en la UI: actualizar la pantalla token a token',
      type: 'reading',
      difficulty: 'profesional',
      content: `## El stream funciona; ahora falta mostrarlo bien\n\nLas lecciones anteriores resolvieron transporte —cómo llegan los tokens del servidor al cliente sin perder ni duplicar nada. Queda un problema distinto: **cómo actualizar la pantalla** a medida que llegan, sin que la interfaz se sienta entrecortada ni se rompa a mitad de una palabra o un bloque de markdown.\n\n## Actualizar estado en cada token, sin destrozar el rendimiento\n\nEl error más común: llamar a \`setState\` en cada token individual sin ningún control, lo que dispara un re-render de React por cada fragmento —a veces decenas por segundo—. React 18 agrupa (batchea) actualizaciones dentro de un mismo ciclo en la mayoría de los casos, pero en un callback de streaming fuera del ciclo normal de eventos conviene ser explícito:\n\n\`\`\`tsx\nfunction useStreamDeTexto(url: string) {\n  const [texto, setTexto] = useState('');\n\n  useEffect(() => {\n    const controller = new AbortController();\n\n    async function leer() {\n      const respuesta = await fetch(url, { signal: controller.signal });\n      const reader = respuesta.body!.getReader();\n      const decoder = new TextDecoder();\n\n      while (true) {\n        const { done, value } = await reader.read();\n        if (done) break;\n        const fragmento = decoder.decode(value, { stream: true });\n        // Actualizacion funcional: evita depender de un 'texto' capturado\n        // en un closure viejo si llegan varios fragmentos muy seguidos\n        setTexto((anterior) => anterior + fragmento);\n      }\n    }\n\n    leer();\n    return () => controller.abort();\n  }, [url]);\n\n  return texto;\n}\n\`\`\`\n\nLa forma funcional de \`setState\` —recibir el valor anterior como argumento, en vez de leer la variable capturada por el closure— no es un detalle de estilo: con actualizaciones que llegan muy seguido, leer directamente la variable de estado puede perder fragmentos si dos llegan antes de que React re-renderice entre medio.\n\n## Markdown a mitad de generar: el problema de los bloques sin cerrar\n\nSi el texto que llega se renderiza como markdown, un bloque de código o una lista pueden quedar **a medio abrir** durante buena parte del streaming —tres backticks que abrieron un bloque de código, pero los tres que lo cierran todavía no llegaron—. La mayoría de los parsers de markdown, sin ayuda, no saben qué hacer con eso: pueden romper el layout momentáneamente o mostrar los backticks como texto plano hasta que el bloque cierra.\n\nLas dos soluciones habituales: usar un parser tolerante a markdown incompleto (varios lo soportan explícitamente para este caso de uso), o renderizar como texto plano mientras el stream está activo y recién parsear como markdown completo cuando el evento de cierre llega. La segunda es más simple de implementar y suficiente para la mayoría de los productos.\n\n## Auto-scroll sin secuestrar el scroll del usuario\n\nUn chat que hace auto-scroll agresivamente hacia abajo en cada token es frustrante si el usuario se desplazó hacia arriba para releer algo — el scroll lo interrumpe constantemente. La solución estándar: detectar si el usuario está cerca del final antes de auto-scrollear, y dejar de hacerlo automáticamente en cuanto el usuario se desplaza manualmente hacia arriba, hasta que vuelva a acercarse al final por su cuenta.`,
      tasks: [
        'Implementa el hook de consumo de stream con actualización funcional de estado',
        'Provoca una ráfaga rápida de fragmentos (reducí el heartbeat del servidor a milisegundos, solo para probar) y confirma que no se pierde ningún fragmento',
        'Implementa el auto-scroll condicional: que se detenga cuando el usuario se desplaza hacia arriba',
      ],
      tip: 'Si tu renderizado de markdown se rompe visualmente durante el streaming pero se ve bien al terminar, es casi siempre un bloque sin cerrar —código o lista— y no un bug real del parser. Renderizar como texto plano mientras el stream está activo suele ser la solución más simple.',
      completed: false,
    },
    {
      id: 'realtime-l4',

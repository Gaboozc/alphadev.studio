...
Un cronjob confiable no es el que nunca falla — eso no existe. Es el que **avisa cuando falla, no se solapa consigo mismo, y produce el mismo resultado si se ejecuta dos veces**. Los tres, no uno solo: observabilidad sin idempotencia todavía puede duplicar datos aunque te enteres; idempotencia sin protección de solapamiento todavía puede correr dos veces a la vez y competir por los mismos recursos.`,
    ...
    {
      id: 'async-l4b',
      title: 'Qué pasa cuando todos los reintentos fallan: dead-letter queues',
      type: 'reading',
      difficulty: 'profesional',
      content: `## Lo que las lecciones anteriores dejaron sin resolver\n\nBackoff con reintentos crecientes resuelve fallos transitorios — una API que tarda, una conexión que se cae un momento. Pero, ¿qué pasa cuando un trabajo agota los tres reintentos y **sigue** fallando? Sin una respuesta explícita a esa pregunta, el trabajo se pierde en silencio, o peor: queda reintentándose para siempre, consumiendo recursos sin avanzar nunca.\n\n## La cola de trabajos muertos\n\nUna **dead-letter queue** (DLQ) es una cola separada donde va un trabajo que agotó todos sus reintentos, en vez de desaparecer. No se procesa automáticamente — se queda ahí, visible, esperando revisión humana o un proceso de reparación específico.\n\n\`\`\`python\nfrom redis import Redis\nfrom rq import Queue\n\ncola_principal = Queue("exportaciones", connection=Redis())\ncola_muertos = Queue("exportaciones_fallidas", connection=Redis())\n\ndef procesar_con_dlq(trabajo_id: str, datos: dict, intento: int = 1):\n    try:\n        ejecutar_exportacion(datos)\n    except Exception as error:\n        if intento >= 3:\n            # Se acabaron los reintentos: va a la cola de muertos, no se descarta\n            cola_muertos.enqueue("registrar_fallo_permanente", trabajo_id, datos, str(error))\n            notificar_equipo(trabajo_id, error)\n        else:\n            cola_principal.enqueue_in(\n                timedelta(seconds=10 * (3 ** intento)),\n                procesar_con_dlq, trabajo_id, datos, intento + 1\n            )\n\`\`\`\n\n## Por qué esto importa incluso si nunca revisás la DLQ a mano\n\nEl valor no es solo poder reparar trabajos fallidos manualmente —aunque eso también—. Es que un trabajo que falla permanentemente **deja de reintentarse en un bucle silencioso** que consume recursos sin producir nada. La alternativa —dejar que un trabajo roto reintente para siempre— es indistinguible desde fuera de un sistema sano con mucho tráfico, hasta que alguien nota que la cola nunca baja de tamaño.\n\n## Qué hacer con lo que cae en la DLQ\n\nComo mínimo: una alerta cuando algo llega ahí (nadie revisa una cola que no sabe que existe). Idealmente: un panel o comando que liste los trabajos fallidos con su error, y la posibilidad de reencolarlos manualmente después de corregir la causa —un bug en el código, un dato mal formado, una dependencia que ya se recuperó.`,
      tasks: [
        'Agrega una cola de trabajos muertos a un sistema tuyo con colas: qué pasa hoy con un trabajo que agota los reintentos',
        'Implementa la notificación cuando algo cae en la DLQ — aunque sea un log destacado, no hace falta un sistema de alertas completo todavía',
        'Diseña el proceso de reencolar manualmente un trabajo de la DLQ después de corregir su causa',
      ],
      tip: 'Si tu sistema no tiene DLQ hoy, revisá qué pasa realmente con un trabajo que falla tres veces — en más de un sistema real la respuesta honesta es "desaparece sin que nadie se entere", y eso vale la pena arreglarlo antes de que sea un trabajo que le importaba a un cliente.',
      completed: false,
    },
    {
      id: 'async-l4c',
      title: 'Colas con prioridad: no todos los trabajos pueden esperar igual',
      type: 'reading',
      difficulty: 'profesional',
      content: `## No todos los trabajos pueden esperar igual\n\nUna sola cola trata todos los trabajos por igual: orden de llegada. Eso es exactamente lo que NO querés cuando un usuario está esperando en pantalla el resultado de un trabajo, mientras un reporte mensual programado hace cola delante suyo solo porque se encoló antes.\n\n## Colas separadas por prioridad, no una bandera\n\nLa forma más simple y más confiable de priorizar es tener **colas físicamente separadas**, y hacer que los workers las consuman en el orden correcto:\n\n\`\`\`python\nfrom rq import Queue, Worker\nfrom redis import Redis\n\nconexion = Redis()\ncola_urgente = Queue("urgente", connection=conexion)     # el usuario espera en pantalla\ncola_normal = Queue("normal", connection=conexion)       # puede tardar minutos\ncola_baja = Queue("baja", connection=conexion)           # reportes, batch nocturno\n\n# El worker escucha las tres, mismo el orden importa: rq las procesa\n# en el orden en que se listan cuando ambas tienen trabajos pendientes\nworker = Worker([cola_urgente, cola_normal, cola_baja], connection=conexion)\nworker.work()\n\`\`\`\n\nUn trabajo encolado en \`cola_urgente\` se procesa antes que cualquier trabajo pendiente en \`cola_normal\` o \`cola_baja\`, sin importar el orden de llegada entre colas distintas — dentro de cada cola, sigue siendo FIFO.\n\n## El error de diseño más común: una sola cola con un campo de prioridad\n\nEs tentador agregar un campo \`prioridad\` al trabajo y ordenar la cola por ese campo. En la práctica esto es mucho más difícil de operar bien: la mayoría de los sistemas de colas (RQ incluido) están optimizados para FIFO simple, no para reordenar dinámicamente. Colas separadas logran el mismo resultado con la primitiva que el sistema ya soporta bien, sin reinventar un scheduler.\n\n## Cuántas colas de prioridad hacen falta\n\nTres niveles (urgente, normal, baja) resuelven la gran mayoría de los casos reales. Más niveles que eso rara vez se justifican y complican la operación —hay que decidir, para cada tipo de trabajo nuevo, en cuál de las N colas va, y esa decisión se vuelve más ambigua cuantas más opciones hay.`,
      tasks: [
        'Clasifica los tipos de trabajo de tu sistema en urgente, normal y baja prioridad',
        'Separa tu cola única en tres colas y ajusta el worker para escucharlas en el orden correcto',
        'Provoca una situación con trabajos de las tres prioridades encolados a la vez y confirma que el orden de procesamiento respeta la prioridad',
      ],
      tip: 'Un trabajo urgente que espera detrás de cien trabajos de baja prioridad es, en la práctica, tan malo como no tener sistema de colas — la prioridad solo funciona si de verdad hay suficientes workers dedicados a la cola urgente, no solo la cola separada.',
      completed: false,
    },
    {
      id: 'async-l5',

...
Los dos son compatibles en el mismo sistema: una pizarra central para el caso, y traspasos estructurados puntuales entre dos agentes que necesitan coordinarse en un paso concreto.`,
    ...
    {
      id: 'ie3-l1e',
      title: 'Evaluator-optimizer: un agente que genera, otro que critica',
      type: 'reading',
      difficulty: 'profesional',
      content: `## El patrón que falta en la lista\n\nRouter, orquestador-trabajador y pizarra compartida resuelven cómo repartir trabajo. Ninguno resuelve un problema distinto: **cómo mejorar un resultado que ya se generó, sin depender de que salga bien a la primera**. Para eso existe el patrón evaluator-optimizer — documentado por Anthropic como uno de los flujos agénticos con mejor relación resultado/complejidad que existe.\n\n## La estructura\n\nUn agente **generador** produce una salida. Un agente **evaluador** —con un prompt distinto, a veces un modelo distinto— la juzga contra criterios explícitos y devuelve una crítica estructurada, no un simple apruebo/rechazo. Si no pasa, el generador recibe esa crítica y reintenta. El ciclo se repite hasta que aprueba o hasta un límite de intentos.\n\n\`\`\`python\nfrom typing import TypedDict\n\nclass EstadoRevision(TypedDict):\n    tarea: str\n    borrador: str\n    critica: str | None\n    intentos: int\n    aprobado: bool\n\ndef generar(estado: EstadoRevision) -> dict:\n    if estado.get("critica"):\n        prompt = f"Tarea: {estado['tarea']}. Borrador anterior: {estado['borrador']}. Corrige esto: {estado['critica']}"\n    else:\n        prompt = f"Tarea: {estado['tarea']}"\n    borrador = modelo_generador.invoke(prompt)\n    return {"borrador": borrador, "intentos": estado.get("intentos", 0) + 1}\n\ndef evaluar(estado: EstadoRevision) -> dict:\n    resultado = modelo_evaluador.invoke(\n        f"Evalua este borrador contra los criterios X, Y, Z: {estado['borrador']}"\n    )\n    return {"aprobado": resultado.aprobado, "critica": resultado.critica}\n\ndef decidir(estado: EstadoRevision) -> str:\n    if estado["aprobado"] or estado["intentos"] >= 3:\n        return "fin"\n    return "generar"  # vuelve con la critica en el estado\n\`\`\`\n\n## Por qué el evaluador es un agente distinto, no el mismo que revisa su propio trabajo\n\nPedirle al mismo agente que generó algo que lo evalúe reproduce el mismo sesgo que lo llevó a producir ese resultado — si su noción de "correcto" estaba mal calibrada al generar, va a estar igual de mal calibrada al juzgar. El evaluador funciona mejor con un prompt separado, enfocado exclusivamente en criterios de calidad explícitos, y en los casos de mayor exigencia con un modelo distinto al del generador.\n\n## El límite de intentos no es opcional\n\nSin un tope, un evaluador exigente y un generador que no logra satisfacerlo producen un ciclo que no termina — consumiendo tokens y tiempo sin límite. El límite de intentos (tres es razonable como punto de partida) convierte un posible bucle infinito en, en el peor caso, una respuesta que llega marcada como "no aprobada tras N intentos" en vez de una llamada que nunca vuelve.\n\n## Cuándo vale la pena frente a un solo agente con mejor prompt\n\nSi un prompt mejor resuelve el problema de calidad, ese es siempre el camino más barato — evaluator-optimizer añade latencia y coste real (como mínimo el doble de llamadas). El patrón se justifica cuando los criterios de calidad son verificables de forma más confiable **evaluando** que **generando de una** — código que debe pasar tests, una traducción que debe preservar tono y precisión técnica a la vez, una respuesta legal que no puede tener errores factuales.`,
      tasks: [
        'Identifica una tarea tuya donde evaluar la calidad de una salida es más confiable que generarla bien a la primera',
        'Construye el ciclo generador-evaluador con un límite de 3 intentos',
        'Escribe el prompt del evaluador con criterios explícitos, no un simple "¿está bien esto?"',
        'Verifica qué devuelve tu sistema cuando se agota el límite de intentos sin aprobar',
      ],
      tip: 'Si notás que el generador y el evaluador "se ponen de acuerdo" con demasiada facilidad en la primera pasada, sospechá del prompt del evaluador — un evaluador que aprueba casi todo no está aportando nada que un solo agente no diera ya.',
      completed: false,
    },
    {
      id: 'ie3-l1f',
      title: 'Sistemas jerárquicos: cuando un agente coordina a otros agentes',
      type: 'reading',
      difficulty: 'profesional',
      content: `## Cuando un router ya no alcanza\n\nEl patrón router funciona bien con un puñado de agentes especializados y un clasificador que decide entre ellos. El problema aparece cuando el número de especialistas crece —diez, veinte agentes distintos— y el propio clasificador empieza a confundirse entre opciones parecidas, o cuando la decisión de enrutado en sí misma requiere varios pasos de razonamiento. La respuesta es lo mismo que en cualquier sistema que crece: **otro nivel de jerarquía**.\n\n## La estructura de dos niveles\n\nUn agente **supervisor** no ejecuta trabajo directamente — decide a qué **sub-supervisor** o equipo delegar, y cada sub-supervisor gestiona su propio grupo de agentes especializados con la lógica de router u orquestador-trabajador que ya conoces. El supervisor de nivel superior nunca necesita conocer los detalles internos de cada equipo, solo su propósito general.\n\n\`\`\`python\ndef supervisor_general(estado: Estado) -> str:\n    # Decide entre AREAS grandes, no entre agentes individuales\n    area = clasificador_general.invoke(estado["consulta"])\n    return area  # 'equipo_ventas', 'equipo_soporte', 'equipo_facturacion'\n\ndef supervisor_soporte(estado: Estado) -> str:\n    # Dentro del equipo de soporte, decide entre especialistas\n    especialista = clasificador_soporte.invoke(estado["consulta"])\n    return especialista  # 'agente_tecnico', 'agente_cuenta', 'agente_devoluciones'\n\ngrafo.add_node("supervisor_general", supervisor_general)\ngrafo.add_node("equipo_soporte", subgrafo_soporte)  # un subgrafo entero, ver leccion anterior\ngrafo.add_node("equipo_ventas", subgrafo_ventas)\ngrafo.add_conditional_edges("supervisor_general", enrutar_area, ["equipo_soporte", "equipo_ventas"])\n\`\`\`\n\nCada \`equipo_*\` de este ejemplo es, en la práctica, un subgrafo compilado —el mismo mecanismo de composición que ya viste en LangGraph—, lo que hace que jerárquico y modular sean, en el fondo, la misma idea aplicada dos veces.\n\n## El coste real de este patrón\n\nCada nivel de jerarquía añade al menos una llamada más al modelo antes de llegar al agente que finalmente resuelve la tarea — el supervisor general clasifica, el sub-supervisor clasifica de nuevo, y recién ahí un especialista trabaja. Para la mayoría de los sistemas, un router de un solo nivel con diez o menos opciones bien diferenciadas resuelve el problema sin esa latencia extra. La jerarquía se justifica cuando el número de especialistas es genuinamente grande, o cuando agrupar por área reduce errores de clasificación que un clasificador plano no distinguiría bien.`,
      tasks: [
        'Cuenta cuántos agentes especializados distintos maneja tu sistema hoy — si son más de ocho o nueve, es candidato a un segundo nivel',
        'Agrupa tus agentes actuales en 2-3 áreas lógicas que un supervisor general podría distinguir sin ambigüedad',
        'Diseña (no hace falta implementar) el prompt del clasificador de nivel superior, y verifica que sus categorías no se superpongan',
      ],
      tip: 'Antes de agregar un nivel de jerarquía, medí cuántas veces tu router plano actual clasifica mal. Si el problema es real (confunde categorías parecidas seguido), la jerarquía ayuda. Si el router plano funciona bien, el nivel extra solo suma latencia.',
      completed: false,
    },
    {
      id: 'ie3-l4',

...
- ¿Qué pasa si su descripción cambia mañana sin que tú lo notes?`,
    ...
    {
      id: 'mcp-l4b',
      title: 'Conectar un agente a un servidor MCP: el lado cliente',
      type: 'reading',
      difficulty: 'profesional',
      content: `## El lado que faltaba\n\nHasta ahora construiste el servidor. Pero la mitad del valor de MCP es la otra punta: conectar un agente tuyo a servidores que **ya existen** —de terceros, o el que acabas de construir— sin escribir el cliente de integración a mano. Aquí es donde MCP deja de ser una idea y se vuelve la herramienta que usás todos los días.\n\n## Conectar desde LangChain / LangGraph\n\n\`\`\`python\nfrom langchain_mcp_adapters.client import MultiServerMCPClient\n\ncliente = MultiServerMCPClient({\n    "archivos": {\n        "transport": "stdio",\n        "command": "npx",\n        "args": ["-y", "@modelcontextprotocol/server-filesystem", "/ruta/permitida"],\n    },\n    "buscador": {\n        "transport": "streamable_http",\n        "url": "https://mcp.ejemplo.com/mcp",\n    },\n})\n\nherramientas = await cliente.get_tools()\n# herramientas es una lista de tools de LangChain, listas para bind_tools()\nmodelo_con_tools = modelo.bind_tools(herramientas)\n\`\`\`\n\nLo notable de este bloque: \`herramientas\` sale con la misma forma que las herramientas que definiste a mano con \`@tool\` en el módulo de fundamentos. Desde la perspectiva de tu agente, una herramienta MCP de un servidor de terceros y una función Python local **son indistinguibles** — se bindean igual, se invocan igual, aparecen igual en el registro.\n\n## Un cliente puede hablar con varios servidores a la vez\n\nEl ejemplo conecta dos servidores —uno local por stdio, otro remoto por HTTP— y expone las herramientas de ambos como una sola lista. Esto es lo que resuelve de verdad el problema de N×M de la primera lección: tu agente no necesita saber que hay dos servidores distintos con protocolos de conexión distintos, solo ve una lista de herramientas.\n\n## Qué revisar antes de conectar un servidor de terceros\n\nTodo lo de la lección de seguridad aplica igual desde el lado cliente: las descripciones de las herramientas que trae un servidor externo son texto que tu modelo va a leer como si fuera confiable. Antes de conectar un servidor que no construiste, revisa qué herramientas expone, qué permisos necesita cada una, y —si el servidor no es de un proveedor que conoces— trátalo con la misma sospecha que un paquete de npm recién publicado.`,
      tasks: [
        'Conecta tu agente al servidor MCP que construiste en el módulo de fundamentos usando MultiServerMCPClient',
        'Agrega un segundo servidor (puede ser uno público de ejemplo) y confirma que las herramientas de ambos aparecen en la misma lista',
        'Audita las herramientas de un servidor de terceros antes de conectarlo: qué hace cada una, qué permisos pide',
      ],
      tip: 'Si vas a conectar varios servidores MCP a un mismo agente, vigila que no haya dos herramientas con nombres o propósitos parecidos entre servidores distintos — el modelo puede confundirlas igual que confundiría dos funciones locales mal nombradas.',
      completed: false,
    },
    {
      id: 'mcp-l4c',
      title: 'Sampling: cuando el servidor le pide algo al modelo del cliente',
      type: 'reading',
      difficulty: 'profesional',
      content: `## Una dirección del protocolo que no viste todavía\n\nTodo lo anterior fue el cliente pidiéndole cosas al servidor: llamar una tool, leer un resource. **Sampling** es el camino inverso: un servidor MCP le pide al cliente que genere una respuesta con SU modelo, en medio de ejecutar una herramienta.\n\n## Por qué existiría esa necesidad\n\nUn servidor MCP de análisis de documentos podría necesitar resumir un fragmento largo antes de decidir su siguiente paso interno —pero el servidor mismo no tiene acceso a un modelo, ni debería necesitar su propia API key de un proveedor de LLM. Sampling le permite pedirle esa generación al cliente, que sí tiene el modelo (y las credenciales) configurados.\n\n\`\`\`python\n# Del lado del servidor: pide una generacion al cliente en medio de una tool\n@servidor.tool()\nasync def analizar_documento(contenido: str, ctx: Context) -> dict:\n    resumen = await ctx.session.create_message(\n        messages=[{"role": "user", "content": f"Resume en 3 frases: {contenido}"}],\n        max_tokens=150,\n    )\n    # el cliente decide con QUE modelo responder, el servidor no lo sabe ni lo controla\n    return {"resumen": resumen.content, "longitud_original": len(contenido)}\n\`\`\`\n\n## El control se queda del lado del cliente, a propósito\n\nEl cliente puede —y en una implementación seria, debe— mostrarle la solicitud al usuario antes de ejecutarla, elegir qué modelo usar para responder, o rechazar la solicitud directamente. Un servidor no puede forzar una generación ni elegir el modelo que la produce; solo puede pedirla. Este diseño evita que un servidor MCP se convierta en una vía para gastar la cuenta de API del cliente sin que este lo autorice.\n\n## El estado real de soporte\n\nSampling es una de las partes más recientes de la especificación, y no todos los hosts MCP la implementan todavía del mismo modo —revisa la documentación del cliente concreto (Claude Desktop, tu propio host) antes de depender de esta función en un servidor que vayas a distribuir a terceros.`,
      tasks: [
        'Explica con tus palabras por qué el servidor no debería tener su propia API key de un proveedor de LLM',
        'Diseña (sin implementar) un caso en tu propio servidor MCP donde sampling resolvería algo que hoy no puede resolver',
        'Revisa la documentación del host o cliente MCP que uses hoy: ¿soporta sampling? ¿qué control le da al usuario sobre esas solicitudes?',
      ],
      tip: 'Sampling es la parte de MCP donde más vale leer la especificación oficial actualizada en vez de confiar en un ejemplo fijo — es la sección que más está evolucionando entre versiones del protocolo.',
      completed: false,
    },
    {
      id: 'mcp-l5',

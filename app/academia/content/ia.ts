...
El sistema funciona sin que tú hagas nada manualmente, produce outputs de calidad consistente, y el tiempo que ahorra por semana > el tiempo que tardó en construirse.`,
  ...
  },
  {
    id: 'ia-7',
    number: 66,
    title: 'De Prompting a Código: el puente hacia Ingeniería de IA',
    description: 'El paso que separa usar herramientas de IA de construir con ellas: Python real, LLMs integrados en tu propio código, y los fundamentos de RAG. El módulo que conecta esta rama con Ingeniería de IA.',
    duration: '2 semanas',
    status: 'available',
    track: 'ia',
    lessons: [
      {
        id: 'ia7-l1',
        title: 'Python para IA: librerías, notebooks y entornos',
        type: 'reading',
        content: `## Por qué Python, y no el lenguaje que ya conoces\n\nCasi todo el ecosistema de IA (LangChain, OpenAI SDK, Anthropic SDK, Hugging Face, las librerías de embeddings y vector DBs) publica su versión más completa y mejor documentada en Python primero. No es una preferencia estética — es dónde vive la comunidad, los tutoriales y los ejemplos que vas a necesitar copiar y adaptar en tus primeras semanas.\n\n## El entorno mínimo para empezar\n\n**Un gestor de entornos virtuales**: venv (incluido con Python) o uv (más rápido, cada vez más adoptado). Sin un entorno virtual, las dependencias de un proyecto contaminan las de otro — el primer error que comete casi todo el mundo es instalar todo globalmente y después no poder reproducir qué versión de qué librería usaba cada proyecto.\n\n\`\`\`bash\npython -m venv .venv\nsource .venv/bin/activate  # Mac/Linux\npip install openai anthropic python-dotenv\n\`\`\`\n\n**Notebooks (Jupyter) vs scripts**: un notebook (.ipynb) es ideal para explorar — ejecutar celda por celda, ver resultados intermedios, iterar rápido sobre un prompt o una consulta. Un script (.py) es lo que corre en producción — determinístico, sin estado oculto entre celdas que se ejecutaron en un orden distinto al que aparecen en pantalla. La secuencia normal: explorás en un notebook, y cuando el flujo funciona, lo pasás a un script limpio.\n\n**Variables de entorno para las API keys**: nunca una API key escrita directo en el código. Un archivo .env (agregado a .gitignore, nunca commiteado) más la librería python-dotenv para cargarlo.\n\n\`\`\`python\nfrom dotenv import load_dotenv\nimport os\n\nload_dotenv()\napi_key = os.getenv('ANTHROPIC_API_KEY')\n\`\`\`\n\n## Las librerías que vas a usar en este módulo\n\nanthropic y openai: los SDKs oficiales para llamar a los modelos por código en vez de por una interfaz de chat. python-dotenv: cargar configuración sin hardcodear secretos. Más adelante en este módulo: langchain (orquestación de LLMs) y un cliente de base de datos vectorial.`,
        tasks: [
          'Crea un entorno virtual nuevo, instala anthropic y python-dotenv, y verifica que pip list muestra ambas instaladas',
          'Configura tu API key en un archivo .env y cárgala con python-dotenv en un script de una línea que la imprima (parcialmente, por seguridad) para confirmar que se leyó bien',
          'Explica con tus palabras por qué un notebook es buena herramienta para explorar pero mala idea para el código que termina en producción',
        ],
        tip: 'Si nunca usaste la terminal más allá de lo básico, este es el momento de perder el miedo — todo el trabajo de este módulo pasa por ahí. Un entorno virtual roto se arregla borrando la carpeta .venv y creando uno nuevo, así que no hay forma de romper nada de forma permanente.',
        completed: false,
      },
      {
        id: 'ia7-l2',
        title: 'De prompts a funciones: envolver un LLM en código real',
        type: 'reading',
        content: `## La diferencia entre usar ChatGPT y construir con un LLM\n\nEscribir un prompt en una interfaz de chat es una conversación de una sola vez. Envolver ese mismo prompt en una función de Python lo convierte en algo reutilizable, testeable, y combinable con el resto de tu código — la base de cualquier producto real con IA.\n\n## Tu primera función que llama a un LLM\n\n\`\`\`python\nfrom anthropic import Anthropic\n\nclient = Anthropic()  # lee ANTHROPIC_API_KEY del entorno automaticamente\n\ndef resumir_texto(texto, max_palabras=50):\n    mensaje = client.messages.create(\n        model='claude-sonnet-4-5',\n        max_tokens=200,\n        messages=[{\n            'role': 'user',\n            'content': f'Resume este texto en maximo {max_palabras} palabras: {texto}'\n        }]\n    )\n    return mensaje.content[0].text\n\n# Ahora es una funcion normal, testeable como cualquier otra\nresumen = resumir_texto('...', max_palabras=30)\n\`\`\`\n\nEsta función ya se puede llamar desde un endpoint web, un script programado, o desde otra función más grande — es exactamente la diferencia entre saber usar IA y construir con IA.\n\n## Salida estructurada: cuando necesitas datos, no prosa\n\nPara la mayoría de los casos de uso reales (clasificar, extraer datos, decidir un siguiente paso) necesitás que la respuesta del modelo sea un dato que tu código pueda usar directamente, no un párrafo que hay que parsear a mano.\n\n\`\`\`python\nimport json\n\ndef clasificar_sentimiento(texto):\n    esquema = '{"sentimiento": "positivo|negativo|neutral", "confianza": 0.0-1.0}'\n    mensaje = client.messages.create(\n        model='claude-sonnet-4-5',\n        max_tokens=100,\n        messages=[{\n            'role': 'user',\n            'content': f'Clasifica el sentimiento. Responde SOLO con este JSON: {esquema}. Texto: {texto}'\n        }]\n    )\n    return json.loads(mensaje.content[0].text)\n\`\`\`\n\nDefinir el esquema JSON como un string aparte (con comillas simples afuera y dobles adentro, sin necesidad de escapar nada) y después referenciarlo dentro del f-string es más legible que anidar comillas escapadas directo en el prompt. Para producción real, librerías como instructor o el soporte nativo de tool use de cada proveedor dan garantías más fuertes de que la salida siempre tiene la forma esperada — algo que verás en profundidad si continuás hacia la rama de Ingeniería de IA.\n\n## Manejo de errores: un LLM también puede fallar\n\nUna llamada a la API puede fallar por rate limits, timeouts, o una respuesta que no vino en el formato esperado. Código que asume que la llamada siempre funciona se rompe en producción la primera vez que la red falla.\n\n\`\`\`python\nimport time\n\ndef llamar_con_reintento(func, intentos=3):\n    for intento in range(intentos):\n        try:\n            return func()\n        except Exception:\n            if intento == intentos - 1:\n                raise\n            time.sleep(2 ** intento)  # backoff exponencial\n\`\`\``,
        tasks: [
          'Escribe la función resumir_texto del ejemplo y pruébala con 2 textos distintos de largo variable',
          'Escribe una función que clasifique un texto en una categoría (por ejemplo: queja, pregunta, elogio) devolviendo un dict de Python, no un string',
          'Agrega manejo de errores básico (try/except) a una de tus funciones para que no se caiga si la API tarda o falla',
        ],
        tip: 'El error más común al empezar: pedirle al modelo JSON y asumir que json.loads siempre va a funcionar. A veces el modelo agrega texto antes o después del JSON — envolver el parseo en un try/except, o pedirle explícitamente que responda Únicamente con el JSON, sin texto adicional, reduce mucho este problema.',
        completed: false,
      },
      {
        id: 'ia7-l3',
        title: 'RAG desde cero: embeddings, vector DB y estrategias de recuperación',
        type: 'reading',
        content: `## El problema que RAG resuelve\n\nUn LLM solo sabe lo que aprendió durante su entrenamiento — no conoce los documentos internos de tu empresa, ni datos que cambiaron después de esa fecha de corte. RAG (Retrieval-Augmented Generation) resuelve esto buscando información relevante ANTES de generar la respuesta, y agregándola al prompt como contexto.\n\n## Embeddings: convertir texto en números que capturan significado\n\nUn embedding es un vector (una lista de números) que representa el significado de un texto — textos con significado parecido producen vectores matemáticamente cercanos, sin importar si usan las mismas palabras exactas.\n\n\`\`\`python\nfrom openai import OpenAI\n\nclient = OpenAI()\n\ndef obtener_embedding(texto):\n    respuesta = client.embeddings.create(\n        model='text-embedding-3-small',\n        input=texto\n    )\n    return respuesta.data[0].embedding\n\n# 'como cancelo mi suscripcion' y 'quiero dar de baja mi plan'\n# producen embeddings muy cercanos, aunque no comparten ninguna palabra\n\`\`\`\n\n## Vector databases: buscar por significado, no por texto exacto\n\nUna base de datos vectorial guarda estos embeddings y permite buscar los N documentos más parecidos en significado a una pregunta — una búsqueda por similitud de coseno o distancia euclídea, no un LIKE %palabra% de SQL tradicional.\n\nSupabase tiene soporte nativo para esto con la extensión pgvector, lo que significa que podés guardar embeddings en la misma base de datos Postgres que ya usás para el resto de tu app, sin sumar un servicio separado.\n\n\`\`\`sql\n-- Habilitar pgvector en Supabase (una vez, desde el SQL Editor)\ncreate extension if not exists vector;\n\ncreate table documentos (\n  id bigint primary key generated always as identity,\n  contenido text,\n  embedding vector(1536)\n);\n\n-- Busqueda por similitud: los 5 documentos mas parecidos a un embedding dado\nselect contenido, 1 - (embedding <=> $1) as similitud\nfrom documentos\norder by embedding <=> $1\nlimit 5;\n\`\`\`\n\n## El flujo completo de RAG, de punta a punta\n\n1. **Indexar** (una vez, o cada vez que cambian los documentos): dividir los documentos en fragmentos (chunks), generar el embedding de cada fragmento, guardarlo en la base vectorial\n2. **Recuperar** (en cada consulta del usuario): generar el embedding de la pregunta, buscar los N fragmentos más similares\n3. **Generar**: armar un prompt con la pregunta + los fragmentos recuperados como contexto, y pedirle al LLM que responda basándose SOLO en ese contexto\n\n\`\`\`python\ndef responder_con_rag(pregunta):\n    embedding_pregunta = obtener_embedding(pregunta)\n    fragmentos = buscar_similares(embedding_pregunta, limite=5)\n\n    contexto = ' '.join(fragmentos)\n    mensaje = client.messages.create(\n        model='claude-sonnet-4-5',\n        max_tokens=500,\n        messages=[{\n            'role': 'user',\n            'content': f'Contexto: {contexto}. Pregunta: {pregunta}. Responde solo con informacion del contexto. Si no esta ahi, dilo explicitamente.'\n        }]\n    )\n    return mensaje.content[0].text\n\`\`\`\n\n## Por qué el tamaño del chunk importa\n\nFragmentos muy grandes diluyen la relevancia (el embedding representa un promedio de mucho contenido distinto). Fragmentos muy chicos pierden contexto (una frase suelta sin el párrafo que la explica). El punto de partida razonable para la mayoría de los casos: chunks de 300-500 palabras, con un solape de 50-100 palabras entre chunks consecutivos para no cortar ideas a la mitad.`,
        tasks: [
          'Genera el embedding de 3 frases con significado similar pero palabras distintas, y de una frase con significado distinto — confirma que las similares quedan más cerca',
          'Crea la tabla de documentos con pgvector en un proyecto de Supabase (o uno de práctica) y guarda 5 fragmentos de texto con sus embeddings',
          'Escribe la función responder_con_rag completa y pruébala con una pregunta que SÍ tenga respuesta en tus documentos, y otra que NO la tenga — verifica que en el segundo caso el modelo lo diga explícitamente en vez de inventar una respuesta',
        ],
        tip: 'Si vas a continuar hacia la rama de Ingeniería de IA después de este módulo, esto profundiza exactamente el módulo de Modelos, embeddings y RAG de esa rama — estrategias de chunking más sofisticadas, re-ranking, y RAG híbrido.',
        completed: false,
      },
      {
        id: 'ia7-proj',
        title: 'Proyecto: chatbot RAG funcional con LangChain y Supabase',
        type: 'project',
        difficulty: 'intermedio',
        projectBrief: 'Construye un chatbot funcional que responda preguntas basándose en un conjunto de documentos propio (puede ser: la documentación de tu agencia, las FAQ de un cliente, o cualquier corpus de texto real que tengas a mano). El entregable es el sistema funcionando, no un diagrama de cómo funcionaría.',
        deliverables: [
          'Al menos 10 documentos o fragmentos indexados en Supabase con pgvector',
          'Función de recuperación que, dada una pregunta, devuelve los fragmentos más relevantes',
          'Chatbot funcional (puede ser un script de terminal, no hace falta una interfaz web) que responde preguntas usando RAG',
          'Manejo explícito del caso en que la respuesta no está en los documentos — el bot debe decirlo, no inventar',
          'Al menos 5 preguntas de prueba documentadas con la respuesta que dio el bot, incluyendo una pregunta sin respuesta en el corpus',
        ],
        tasks: [
          'Implementa el pipeline completo: indexar, recuperar, generar',
          'Prueba el bot con preguntas que sabés que tienen respuesta en tus documentos y con preguntas que no la tienen',
          'Documenta en un README qué corpus usaste, qué tamaño de chunk elegiste y por qué',
        ],
        tip: 'No hace falta LangChain para que esto funcione — el ejemplo de este módulo usa los SDKs directos, que para un proyecto de este tamaño son más simples de debuggear. Si querés usar LangChain, este proyecto es un buen lugar para probarlo por primera vez sin la presión de un sistema en producción.',
        completed: false,
      },
    ],
    resources: [
      { title: 'Anthropic SDK para Python — documentación oficial', url: 'https://docs.claude.com/en/api/client-sdks', type: 'documentation' },
      { title: 'Supabase — pgvector: embeddings y búsqueda semántica', url: 'https://supabase.com/docs/guides/ai/vector-columns', type: 'documentation' },
      { title: 'LangChain — documentación oficial', url: 'https://python.langchain.com', type: 'documentation' },
    ],
  }
]